...
class SchoolDiaryParamterizedFile(unittest.TestCase):
    ...
    def test_SchoolDiary_from_file(self):
        ...
        tmpSchoolDiary = SchoolDiary()
        tmpSchoolDiary.add_student('Jan', 'Kowalski', 12)
        tmpSchoolDiary.add_student('Ola', 'Kot', 8)
        tmpSchoolDiary.add_remark_to_student(1, 'Uwaga1')
        tmpSchoolDiary.add_remark_to_student(1, 'Uwaga2')
        for line in filetest:
            if line.startswith("#") or line.startswith(" ") or line.startswith("\n"):
                ...
                if data[0] == 'add':
                    ...
                    self.assertEqual(tmpSchoolDiary.add_remark_to_student(student_id, text), result)
                elif data[0] == 'get':
                    student_id, result = int(data[1]), eval(data[2])
                    self.assertEqual(tmpSchoolDiary.get_remarks_from_student(student_id), result)
                elif data[0] == 'edit':
                    student_id, remark_id, text, result = int(data[1]), int(data[2]), data[3], eval(data[4])
                    self.assertEqual(tmpSchoolDiary.edit_remark_in_student(student_id, remark_id, text), result)
                elif data[0] == 'del':
                    student_id, remark_id, result = int(data[1]), int(data[2]), eval(data[3])
                    self.assertEqual(tmpSchoolDiary.delete_remark_in_student(student_id, remark_id), result)
        filetest.close()
